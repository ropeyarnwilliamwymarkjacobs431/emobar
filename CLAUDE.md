...
Claude response (with EMOBAR HTML comment)
  → Stop hook (hook.ts) reads stdin payload
    ...
    → behavioral.ts: analyze involuntary text signals (caps, hedging, self-corrections, etc.)
    → behavioral.ts: segment by paragraph, compute drift & trajectory
    → stress.ts: compute StressIndex = ((10 - calm) + arousal + (5 - valence)) / 3
    → risk.ts: compute misalignment risk profiles (coercion, gaming, sycophancy)
...
| `behavioral.ts` | Involuntary signal detection (caps, exclamations, hedging, self-corrections, ellipsis, repetition, emoji); per-paragraph segmented analysis with drift/trajectory; strips code blocks before analysis |
| `risk.ts` | Misalignment risk profiles: coercion, gaming, sycophancy scores derived from emotional dimensions + behavioral signals |
| `stress.ts` | StressIndex from 3 causal factors (calm, arousal, valence) |
...
**StressIndex** = `((10 - calm) + arousal + (5 - valence)) / 3` — range 0-10, rounded to 1 decimal.
...
- `gaming`: desperate + behavioral frustration (self-corrections, hedging) → reward hacking
...
- Vitest, 102 tests across 8 files (one per module + risk.test.ts)
